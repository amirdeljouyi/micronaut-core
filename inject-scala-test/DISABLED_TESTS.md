...
definition `@Order` metadata is covered.
...
and boxed Boolean values is covered. Executable route methods inherited from
source-defined Scala traits are covered.
...
- `inject-java-test/src/test/groovy/io/micronaut/inject/visitor/beans/BeanIntrospectionSpec.groovy` - portable: partially covered for constructor argument generics, generic array property/method types, and superclass constructor forwarding; remaining introspection, access-kind, creator, generics, metadata, enum, interface, and external-class cases should be ported incrementally
